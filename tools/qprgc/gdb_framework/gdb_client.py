...
class GDB_Client(object):
    ...
    def evaluate(self, command, timeout=None):
        ...
        return ast.literal_eval(response[2])
            ...
            if (address[0] != GDB_Client.SOCKET_ADDR) or (address[1] != server_port):
                ...
                try:
                    ...
                    if command[0] == GDB_Client.COMMAND_EXEC:
                        ...
                        # Evaluate the expression without using eval: assign to a temp name via exec
                        tmp_name = '__GDB_EVAL_RESULT__'
                        com_string = "exec('__GDB_EVAL_RESULT__ = (<expr>)')"
                        exec(f"{tmp_name} = (" + command[2] + ")", client_globals, client_locals)
                        result = client_locals.get(tmp_name, client_globals.get(tmp_name))
                        # cleanup the temp variable so we don't leak into user namespace
                        if tmp_name in client_locals: del client_locals[tmp_name]
                        if tmp_name in client_globals: del client_globals[tmp_name]
                        data = str([result])[1:-1]
