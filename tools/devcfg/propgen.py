      ...
      for node in global_defs.childNodes:
         ...
         if type == 1: #DALPROP_DATA_TYPE_STRING
            ...
            for index in range (len(value)-1):
               try:
                  value[index] = ast.literal_eval(value[index].lstrip())
               except NameError:
   ...
   for Props in node.getElementsByTagName("props"):
      ...
      if(len(name) != 0):
         ...
         if(nameoffset.isdigit() is True):
            nameoffset = ast.literal_eval(nameoffset)
      ...
      if(len(array) and "True".lower() in array.lower() ):
         instance_is_array =1
         re.sub(r"(\[\.*\])", '', type1) # get rid of '[]' if client mistakenly added in type
      ...
      for child in Props.childNodes:
         if child.nodeType == child.COMMENT_NODE: # skip, if its is a comment node
            continue
         char_data = char_data + re.sub(r"(\s*|\n)", '', child.data)
      
      if type1 == 0x02:   #DALPROP_ATTR_TYPE_UINT32
         try:
            value = ast.literal_eval(char_data)
         except NameError:
            value= str(char_data)
         ...
         for index in range(len(value) - 1):
            ...
            value[index] = ast.literal_eval(value[index])   
         value.insert(0, len(value) - 1)
...
def main():
   ...
   if DirName and ConfigFileName:
      ConfigFileName = DirName + ConfigFileName
   if DirName and DevcfgDataFileName:
      DevcfgDataFileName = DirName + DevcfgDataFileName
   print(DirName)
