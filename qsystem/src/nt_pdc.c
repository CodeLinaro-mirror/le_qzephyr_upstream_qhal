 ...
 */
pdc_resource* nt_pdc_create_resource( pdc_resource_definition *definition,
                                     pdc_resource_state initial_state )
{
  ...
  if( PDC_TRUE ==  nt_pdc_vote_processing(resource, client, vote))
  {
	  ...
	  if( active_agg_vote  != resource->active_state )
	  {
		  resource->active_state =
				  resource->definition->driver_fcn( active_agg_vote, client);
	  }
  }
 ...
 */
pdc_client* pdc_create_client(  char *client_name,
                                  char *resource_name , uint8_t client_type )
{
  pdc_resource *resource;
  pdc_client *client;
