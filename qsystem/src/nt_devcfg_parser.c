...
void nt_devcfg_parse() {
  ...
  if (device_id == 0x03000002) // log params
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        ...
        params_remain =
            GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t, NT_DEVCFG_LFS_REG_TYPE);
  ...
  if (device_id == 0x03000003) // roaming params
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain = GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t,
                                            NT_DEVCFG_FG_SCAN_PROBE_TYPE);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000004) // becon/power params
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain =
            GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t, NT_DEVCFG_INTVAL);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000005) // powersave twt
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain =
            GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t, NT_DEVCFG_TWT_ENABLE);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000008) // wifi protection
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain = GET_NUM_LEFT_DEVCFG(
            nt_devcfg_structure_t, NT_DEVCFG_WIFISEC_AUTH_INTER_FRM_TIMER);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000009) // connection
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain =
            GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t, NT_DEVCFG_CONN_TIMEOUT);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000010) // connection
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain =
            GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t, NT_DEVCFG_WMMP_BE_ACM);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  if (device_id == 0x03000011) // mac
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain = GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t,
                                            NT_DEVCFG_DEFAULT_MAC_ADDR_AP_P1);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
  ...
  //		{
  //			starting_add=(((int)(DALPROP_PropBin_devcfg_xml[count_1+1]))/4);
  //			for(count_2=starting_add; count_2<total_hex_val;
  //count_2+=2)
  //			{
  //				ret_val=DALPROP_PropBin_devcfg_xml[count_2+1];
  //				*count_ptr=ret_val;
  ...
  if (device_id == 0x03000014) // NEUT_2
  {
    ...
    for (count_1 = 0; count_1 < total_hex_val; count_1++) {
      if (DALPROP_PropBin_devcfg_xml[count_1] == device_id) {
        starting_add = (((int)(DALPROP_PropBin_devcfg_xml[count_1 + 1])) / 4);
        params_remain = GET_NUM_LEFT_DEVCFG(nt_devcfg_structure_t,
                                            NT2_DEVCFG_ENABLE_DISABLE_RRAM_DXE);
        end_add = A_MIN(starting_add + params_remain * 2, total_hex_val);
        for (count_2 = starting_add; count_2 < end_add; count_2 += 2) {
          ret_val = DALPROP_PropBin_devcfg_xml[count_2 + 1];
          *count_ptr = ret_val;
...
#ifdef NT_DEBUG
      //	NT_LOG_DEV_CFG_INFO("print value",
      //(ret_dev_data&0x00000f00)>>8,0,0);
#endif
...
//					NT_LOG_DEV_CFG_INFO("a",
//out_ascii[0],0,0); 					NT_LOG_DEV_CFG_INFO("b", out_ascii[1],0,0);
//					NT_LOG_DEV_CFG_INFO("c",
//out_ascii[2],0,0); 					NT_LOG_DEV_CFG_INFO("d", out_ascii[3],0,0);
#endif
