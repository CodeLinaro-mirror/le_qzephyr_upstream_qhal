...
void log_to_buffer_fixed(const char *event_key,
debug_log_elem p1,
debug_log_elem p2,
debug_log_elem p3);

void log_to_buffer_variadic(const char* event_key,
    size_t n_args, ...);

void log_arr_to_buffer(const char* event_key,
    const uint8_t* ptr,
    const uint16_t len);

void log_dynamic_string_to_buffer(const char* event_key,
 char* string_ptr);

void log_dynamic_string_to_buffer_with_len(const char* event_key,
 char* string_ptr, uint16_t string_length);

void log_to_buffer_minimal(const char* event_key,
    size_t n_args, ...);
...
// +-------------------------------+------------------+-----------+
#define NT_LOG_PRINT_FORMAT_CHK(msg,n_args) _Static_assert( LOG_HAS_NO_FMT_SPFRS_AND_NO_ARGS(msg, n_args) || LOG_HAS_FMT_SPFR_AND_ARGS(msg, n_args) ,"Format specifier error!");
#define LOG_HAS_NO_FMT_SPFRS_AND_NO_ARGS(msg, n_args) ((true ==  (n_args==0)) && (false == HAS_NOT_FORMAT_SPECIFIER(msg)))
#define LOG_HAS_FMT_SPFR_AND_ARGS(msg, n_args) ((true == HAS_NOT_FORMAT_SPECIFIER(msg)) && (false ==  (n_args==0)))
#define NT_LOG_MOD_LVL_FORMAT_CHK(msg) _Static_assert((false == (HAS_NOT_FORMAT_SPECIFIER(msg))),"This type of log shall not contain any format specifier!");
