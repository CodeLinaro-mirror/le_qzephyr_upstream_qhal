...
#define NT_STATUS_MAKE(code, mod, submod) ( NT_STATUS_CODE_GET(code) | NT_STATUS_MODULE_GET(mod) | NT_STATUS_SUBMOD_GET(submod))
...
#define NT_AC_DELIVERY_ENABLE_CHECK(notified_ac, txrx_struc) ((1<<(notified_ac)) & txrx_struc->sc_apsdConfig.deliveryEnabled)   //check if notification received AC is uapsd enabled AC
#define NT_EXTRACT_SERVICE_PERIOD(apsd_info)    ((((apsd_info >> IEEE80211_QOSINFO_MAXSP_S) & IEEE80211_QOSINFO_MAXSP_M)*2))
