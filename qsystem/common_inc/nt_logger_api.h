...
#else
#define NT_LOG_PRINT(MODNAME,LVL,msg,...) \
    nt_log_printf(NT_STATUS_MOD_##MODNAME,NT_LOG_LVL_##LVL,(char*)__func__, __LINE__,msg,NUMARGS(__VA_ARGS__),##__VA_ARGS__)
...
#define NT_LOG_ARRAY_PRINT(MODNAME, LVL, msg, ptr, len)                        \
    ...
    {                                                                          \
        if(NT_LOG_LVL_##LVL >= set_loglvl[NT_STATUS_MOD_##MODNAME])            \
        {                                                                      \
        DEBUG_LOG_STRING(log_fmt,  msg""" %s");                                \
...
/**
 * below mentioned macros we are using to create the logs. Goto the code and check the which module we are create the logs.
 * create logs for related module. And we are adding new module logs. Can you please first create a macro for module id (NT_STATUS_MOD_ ...). and
 * update the NT_MAX_MODULE_ID. This two macros available in nt_common.h.
 */
#if ( NT_FN_FUNCTION_LINE_NUM_FLAG == 0 )
 ...
 */
uint8_t nt_log_sub_mod_info(uint8_t  mod_id, uint8_t  loglvl, uint16_t sub_mod_id, const char *msg, uint32_t p1, uint32_t p2, uint32_t p3);
...
#define NT_LOG_MOD_LVL(MODNAME, LVL, msg,p1,p2,p3)                             \
   nt_log_write(NT_STATUS_MOD_##MODNAME, NT_LOG_LVL_##LVL,                     \
                NT_LOG_FILE_LOC                                                \
                msg,p1,p2,p3)
 ...
 */
 ...
 uint8_t nt_log_data_pass_to_buff(log_ringdesc_t *log_buffer,uint8_t mod_id,uint8_t loglvl,
		 ...
		 uint16_t ln,
#endif
		 const char *msg,uint32_t p1,uint32_t p2,uint32_t p3,uint8_t calc_msg_len,uint32_t xStartTime);
  ...
  *   @Description : Through CLI command, we will get the respective log related log information from respective moduleid.
  *   @Param       : moduleid.
  *   				log level.
  *   @return      : Null
  */
