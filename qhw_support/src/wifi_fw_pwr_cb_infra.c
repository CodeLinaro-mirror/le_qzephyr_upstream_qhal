...
fpci_err_t fpci_evt_cb_dereg(ps_evt_cb_t cb, uint16_t evt_reg_mask)
{
    ...
    {
        if ((g_event_bin[itter_index].evt_cb) == cb &&
...
void fpci_register_test_cb(void)
{
    fpci_evt_cb_reg(&ps_evt_cb_dummy1_t,
        PWR_EVT_WMAC_PRE_SLEEP | PWR_EVT_WMAC_POST_SLEEP |
        PWR_EVT_WMAC_PRE_AWAKE | PWR_EVT_WMAC_POST_AWAKE |
        PWR_EVT_WMAC_SLEEP_ABORT, FPCI_CB_DUMMY1_PRIO, NULL);
    fpci_evt_cb_reg(&ps_evt_cb_dummy2_t,
        PWR_EVT_WMAC_PRE_SLEEP | PWR_EVT_WMAC_POST_SLEEP |
        PWR_EVT_WMAC_PRE_AWAKE | PWR_EVT_WMAC_POST_AWAKE |
        PWR_EVT_WMAC_SLEEP_ABORT, FPCI_CB_DUMMY2_PRIO, NULL);
    fpci_evt_cb_reg(&ps_evt_cb_dummy3_t,
        PWR_EVT_WMAC_PRE_SLEEP | PWR_EVT_WMAC_POST_SLEEP |
        PWR_EVT_WMAC_PRE_AWAKE | PWR_EVT_WMAC_POST_AWAKE |
        PWR_EVT_WMAC_SLEEP_ABORT, FPCI_CB_DUMMY3_PRIO, NULL);
}
