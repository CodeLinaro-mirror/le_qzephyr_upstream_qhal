...
void nt_ndxe_deinit(void)
{
	...
	if(halDxe == NULL || !(halDxe->Configured)) return;
...
static eRet_t nt_ndxe_allocate_desc()
{
    ...
    do
    {
        ...
        halDxe->pFreeXfrDescPoolHead = (volatile DescCB_t *)(halDxe->pXfrDescPool);   // Allocated Tx desc starting pointer
...
void nt_ndxe_config_channel(e_dxe_channel channel, p_dxe_channel_cfg_t pcfg)
{
    ...
    pDxeCCB->cw_ctrl_b2h =  (NT_DXE_DESC_CTRL_SIQ |
							NT_DXE_DESC_CTRL_XTYPE_B2H|
							NT_DXE_DESC_CTRL_EOP |
							...
							NT_DXE_DESC_CTRL_PDU_REL |
							NT_DXE_DESC_CTRL_INT |
							NT_DXE_DESC_CTRL_BDT_SWAP);
    ...
    pDxeCCB->chan_mask =    QWLAN_DXE_0_CH0_CTRL_EDVEN_MASK |
    						(channel << QWLAN_DXE_0_CH0_CTRL_CTR_SEL_OFFSET)|
							QWLAN_DXE_0_CH0_CTRL_EDEN_MASK|
							QWLAN_DXE_0_CH0_CTRL_INE_DONE_MASK|
							QWLAN_DXE_0_CH0_CTRL_INE_ERR_MASK|
							QWLAN_DXE_0_CH0_CTRL_INE_ED_MASK|
    ...
    rWrite(pDxeCCB->chDXEBaseAddr + NT_DXE_CH_CTRL_REG, (regVal & ~QWLAN_DXE_0_CH0_CTRL_BTHLD_SEL_MASK) |
															(pDxeCCB->bmuThdSel_mask));
...
#ifdef NT_TST_TIME_STAMP_ENABLE
	if ((nt_dpm_tm.tx_stat[DXE_TX].valid == 0) && (*(uint32_t *)((uint8_t *)frame + NT_TX_BUFFER_OFFSET + nt_dpm_tm.tx_stat[LOW_LVL_OUTPUT].offset) == nt_dpm_tm.tx_marker)) {
...
//		NT_LOG_DPM_CRIT("DXE interrupt error occured",0,0,0);
		...
		//printf("(%s:%d): DXE_0_INT_ERR_SRC = 0x%08x, DXE_0_CH1_STATUS = 0x%08x, DXE_0_CH0_STATUS_REG = 0x%08x\n", __func__, __LINE__, regVal, inp(DXE_0_CH1_STATUS_REG), inp(DXE_0_CH0_STATUS_REG));
		//return NDXE_ERROR_INTERRUPT;
	}
...
uint32_t hal_dxe_suspend()
{
	...
	//	NT_LOG_PRINT(DPM, ERR, "dpm stopped failed dma_csr: 0x%x wmac_status: 0x%x wait_cnt: %d",
    	...
    	NT_LOG_DPM_ERR("dpm stopped failed",HAL_REG_RD(QWLAN_DXE_0_DMA_CSR_REG),HAL_REG_RD(QWLAN_MCU_MCU_WMAC_STATUS_REG),wait_count);
 ...
 */
void __attribute__ ((section(".after_ram_vectors")))
hal_dxe_desc_reconfig(e_dxe_channel channel)
{
    uint8_t desc_idx;
    DescCB_t *pCurrDCB;
    volatile DXEDesc_t *pDXEDesc;
