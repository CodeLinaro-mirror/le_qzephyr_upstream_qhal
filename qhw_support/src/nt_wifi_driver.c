...
wificfg_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state );
static unpa_resource_state
wificfg_update_fcn( unpa_resource *resource, unpa_client *client);
 ...
 */
static unpa_resource_state wificfg_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state )
{
	char wifi_state=0;
	char config_value=0;
	...
	case WIFI_CONFIG:
		...
		NT_REG_WR(QWLAN_PMU_CFG_WIFI_CONFIG_STATE_RESOURCE_REQ_REG,reg_read | (config_value<<SHIFT_REQUESTED_REQ) );
		break;
	case WIFI_TX_STATE:
	...
	case WIFI_RXB_LISTEN:
		...
		reg_read &= SOC_POWER_DOMAIN_CLEAR_MASK;
		NT_REG_WR(QWLAN_PMU_CFG_WIFI_RXB_LISTEN_STATE_RESOURCE_REQ_REG,reg_read | (config_value<<SHIFT_REQUESTED_REQ) );
		break;
	default : 		break;
	}
...
static unpa_resource_state
wifiwur_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state );
static unpa_resource_state
mem_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state );
static unpa_resource_state
phy_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state );
static unpa_resource_state
cpu_mcu_slp_driver_fcn( unpa_resource *resource, unpa_client *client,
                         unpa_resource_state state );

static unpa_resource_state
rram_slp_driver_fcn(unpa_resource *resource, unpa_client *client,
        unpa_resource_state state);

static unpa_resource_state
xip_slp_driver_fcn(unpa_resource *resource, unpa_client *client,
        unpa_resource_state state);

static unpa_resource_state
xip_dp_slp_driver_fcn(unpa_resource *resource, unpa_client *client,
        unpa_resource_state state);

/* Update function declaration */
static unpa_resource_state
wifiwur_update_fcn( unpa_resource *resource, unpa_client *client);

static unpa_resource_state
mem_update_fcn( unpa_resource *resource, unpa_client *client);
...
if ( state != resource->active_state)
{
    ...
    {
    	nt_hw_modify_reg(QWLAN_PMU_CFG_AON_CNTL_WUR_SLEEP_STATE_RESOURCE_REQ_REG,state,SLEEP_STATE_REG_CONFIGURATION_AON);
    	nt_hw_modify_reg(QWLAN_PMU_CFG_AON_CNTL_WIFI_SLEEP_STATE_RESOURCE_REQ_REG,state,SLEEP_STATE_REG_CONFIGURATION_AON);
    }
 ...
 * <!-- phy_driver_fcn -->
 *
 * @brief Driver function for the "phyvr" resource. This driver function will control the bits RFA_DTOP and XO_DTOP based on the user requirement
 * @param resource: Resource for the driver
 * @param client:   Client of the resource
 * @param state:    request which is to taken care of
 * @return required request state
        ...
        unpa_resource_state state)
{
    ...
    else if(client->type == UNPA_CLIENT_SLEEP  )
    {
    	if(state != resource->sleep_state)
    	{
    		...
    		reg_read &= (uint16_t)PHY_DRIVER_MASK;
    	    //NT_REG_WR(QWLAN_PMU_CFG_AON_CNTL_MCU_SLEEP_STATE_RESOURCE_REQ_REG, (reg_read|((resource->sleep_state)<<1)));
 ...
 * <!-- nt_pdc_driver_init -->
 *
 * @brief :  Initialize pdc driver resources and clients for wifi,memory and cpu. This function will initialize all the resources and
 *           clients related to the PDC.
 */
