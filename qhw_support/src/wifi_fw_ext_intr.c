...
void aon_ext_wakeup_set_lvl_trigger(void)
{
    ...
    en_ext_int = NT_REG_RD(NT_NVIC_ISER1);
    en_ext_int &= ~A2F_ASSERT_INTR_NVIC1_MASK;
    NT_REG_WR(NT_NVIC_ISER1, en_ext_int);

    en_ext_int = NT_REG_RD(NT_NVIC_ISER3);
...
#ifdef EXT_WAKE_UP_DEASSERT
    qurt_isr_register_3(AON_cmnss_ext_wakeup_int, aon_ext_interrupt_wake_up_deassert);
    en_ext_int = NT_REG_RD(NT_NVIC_ISER3);
    en_ext_int |= A2F_DEASSERT_INTR_NVIC3_MASK;
    NT_REG_WR(NT_NVIC_ISER3, en_ext_int);
#endif
