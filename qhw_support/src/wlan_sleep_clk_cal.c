...
void socpm_enable_slp_clk_cal_int(void)
{
    ...
    temp1 = temp1 | (0x1 << 1);
    HAL_REG_WR(NT_NVIC_ISER2, temp1);
#endif
    return;
}
...
nt_status_t socpm_slp_clk_cal_get_hbin(void)
{
    ...
    if (hbin_data & QWLAN_PMU_CFG_HBIN0_HBIN_ACTIVE_MASK)
    {
        ...
        {
            NT_LOG_PRINT(SOCPM, ERR, "Xocnt %ld, Refxocnt %ld, RC running Slower ,Bin %d, TS data %ld, TS_data_valid %d",
        ...
        {
            NT_LOG_PRINT(SOCPM, ERR, "Xocnt %ld, Refxocnt %ld, RC running Faster ,Bin %d, TS data %ld, TS_data_valid %d",
                p_slp_clk_cal_params->xocnt, p_slp_clk_cal_params->refxocnt, bin,
                p_slp_clk_cal_params->pmu_temp_sensor_data, is_pmu_ts_data_valid());
        }
    }
    ...
    else
    {
        ...
        {
            p_slp_clk_cal_params->xocnt = (hbin_data & QWLAN_PMU_CFG_HBIN0_CALBIN_DATA_MASK);
            NT_LOG_PRINT(SOCPM, ERR, "SOCPM Sleep Clk Cal - HBIN %d not active, fetching data from hbin %d", bin, (bin - 1));
        }
...
void socpm_sleep_clk_cal_timer_cb(void)
{
    ...
    if ((p_slp_clk_cal_params->pmu_temp_sensor_data >= p_slp_clk_cal_params->hbin_range[p_slp_clk_cal_params->prev_hbin] - TS_HYS_THRESH) &&
        (p_slp_clk_cal_params->pmu_temp_sensor_data < p_slp_clk_cal_params->hbin_range[(p_slp_clk_cal_params->prev_hbin) + 1] + TS_HYS_THRESH)
        && (p_slp_clk_cal_params->sleep_clk_cal_initialized == TRUE))
    {
        return;
    }
    ...
    NT_LOG_PRINT(SOCPM, INFO, "Trigger slp clk cal. %u %u %u %u %u",p_slp_clk_cal_params->pmu_temp_sensor_data,
        ...
        p_slp_clk_cal_params->prev_hbin,
        p_slp_clk_cal_params->sleep_clk_cal_initialized);
#endif /* APPLY_SLEEP_CLK_CORRECTION */
...
nt_status_t  socpm_slp_clk_cal_hw_init(void)
{
    ...
    rdata = (NT_REG_RD(QWLAN_PMU_CFG_SLP_CAL_CAL_EN_REG) & (~QWLAN_PMU_CFG_SLP_CAL_CAL_EN_CFG_SLP_CLK_CAL_SM_GO_IDLE_MASK))
...
nt_status_t socpm_slp_clk_cal_init(void)
{
    ...
    p_slp_clk_cal_params->slp_clk_cal_poll_period = *((uint32_t*)(nt_devcfg_get_config(NT_DEVCFG_SLP_CLK_CAL_POLL_TIMER_PERIOD_MS)));
#ifdef SLEEP_CLK_CAL_IN_SLEEP_MODE
    p_slp_clk_cal_params->sleep_mode_cal_enabled = *((bool*)(nt_devcfg_get_config(NT_DEVCFG_SLP_MODE_SLP_CLK_CAL_ENABLE)));
    ...
    for (uint16_t i = 0; i < (NUM_HBIN_RANGE - 1); i++)
    {
        ...
        {
            p_slp_clk_cal_params->hbin_range[i + 1] = (p_slp_clk_cal_params->hbin_range[i] +
        ...
        {
            p_slp_clk_cal_params->hbin_range[i + 1] = (p_slp_clk_cal_params->hbin_range[i] +
                ((((QWLAN_PMU_CFG_HBIN_6_TO_11_RANGE_CFG_HBIN6_RANGE_MASK) << (HBIN_RANGE_BIT_OFFSET *
                    (i- HBIN_RANGE2_START))) & rdata2) >> (HBIN_RANGE_BIT_OFFSET * (i - HBIN_RANGE2_START))));
        }
        else
        {
            p_slp_clk_cal_params->hbin_range[i + 1] = (p_slp_clk_cal_params->hbin_range[i] +
                ((((QWLAN_PMU_CFG_HBIN_12_TO_15_RANGE_CFG_HBIN12_RANGE_MASK) << (HBIN_RANGE_BIT_OFFSET *
                    (i- HBIN_RANGE3_START))) & rdata3) >> (HBIN_RANGE_BIT_OFFSET * (i - HBIN_RANGE3_START))));
        }
    }
    ...
    p_slp_clk_cal_params->refxocnt = (tmp1 * 9375) / 8; //  (tmp1 * xo_clk_freq/rc_clk_freq) = (tmp1 * 38400000) / 32768;
    ...
    {
        p_slp_clk_cal_params->slp_clk_cal_poll_timer = nt_create_timer(socpm_sleep_clk_cal_timer_cb, NULL,
            INIT_SLP_CAL_POLL_PERIOD_MS, FALSE);
    }
...
void socpm_actv_slp_clk_cal_monitor_pause(void)
{
    ...
    if (NULL != p_slp_clk_cal_params->slp_clk_cal_poll_timer)
    {
        ...
        else
        {
            ...
            NT_LOG_PRINT(SOCPM, INFO, "SOCPM Active Mode Sleep Clock Cal Pause -Poll timer not stopped as its not active");
        }
    }
    ...
    if (NULL != p_slp_clk_cal_params->slp_clk_cal_poll_timer)
    {
        if (p_slp_clk_cal_params->sleep_clk_cal_timer_pending_ticks != 0)
        {
            if (nt_timer_change_time_period(p_slp_clk_cal_params->slp_clk_cal_poll_timer,
                p_slp_clk_cal_params->sleep_clk_cal_timer_pending_ticks) != NT_TIMER_SUCCESS)
            {
...
nt_status_t socpm_slp_clk_cal_enable(slp_clk_cal_mode_t mode)
{
    ...
    if (mode == ACTIVE_MODE)
    {
        ...
        fpci_evt_cb_reg((ps_evt_cb_t)&socpm_actv_slp_clk_cal_slp_cb,
          PWR_EVT_WMAC_PRE_SLEEP | PWR_EVT_WMAC_POST_AWAKE | PWR_EVT_WMAC_SLEEP_ABORT, PS_CB_SLEEP_CAL_PRIORITY, NULL);
#endif
        (void)value;
    }
