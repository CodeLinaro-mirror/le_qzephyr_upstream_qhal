...
uint8_t _socpm_mcu_sleep_wake;

uint8_t _socpm_slp_clk_src;
...
uint32_t aon_cmnss_wlan_slp_tmr_int_cnt = 0;

void Aon_cmnss_wlan_slp_tmr_int(void)
{
    aon_cmnss_wlan_slp_tmr_int_cnt++;
	PROF_IRQ_ENTER();
    ...
    uint64_t sleep_time) // in us
{
    ...
    NVIC_ClearPendingIRQ(AON_cmnss_wlan_slp_tmr_int);
...
uint64_t socpm_imps_min_cb(uint32_t wkup_delay_us)
{
...
static void __attribute__((used)) socpm_enter_mcusleep()
{
    ...
    if (FALSE == g_socpm_struct.in_warm_boot)
    {
        ...
        value = NT_REG_RD(NT_NVIC_ISER2);
        nt_socpm_m4_regs[13] = value;

        value = NT_REG_RD(NT_NVIC_ISER3);
    ...
    void)
{
    ...
    qurt_isr_register_3(AON_cmnss_wlan_slp_tmr_int, Aon_cmnss_wlan_slp_tmr_int);
...
void power_dump_cfg (void)
{
    PRINT_LOG_FUNC_LINE_ENTRY;
    log_printf("_socpm_slp_clk_src=%d\n", _socpm_slp_clk_src);
    log_printf("_socpm_slp_time_supp_min_ms=%d\n", _socpm_slp_time_supp_min_ms);
    log_printf("g_socpm_struct.socpm_indef_deep_sleep_en=%d\n", g_socpm_struct.socpm_indef_deep_sleep_en);
    log_printf("g_socpm_struct.cpr_cfg.ini_enabled=%d\n", g_socpm_struct.cpr_cfg.ini_enabled);
    log_printf("g_socpm_struct.slop_step_us=%d\n", g_socpm_struct.slop_step_us);
    log_printf(" g_socpm_struct.slop_interval_ms=%d\n",  g_socpm_struct.slop_interval_ms);
    ...
    uint64_t sleep_time)
{
    power_dump_cfg();
    if (sleep_time < (_socpm_slp_time_supp_min_ms * 1000)) {
        NT_LOG_PRINT(SOCPM, ERR, "Too small sleep time: %llu us, using min %u us",
            sleep_time,_socpm_slp_time_supp_min_ms * 1000);
