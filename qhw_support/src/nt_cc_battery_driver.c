...
#ifdef FR_HWIO_WAR
#define QWLAN_PMU_CFG_AON_CNTL_MCU_SYSTEM_BOOT_COMPLETE_STATE_RESOURCE_REQ_PD_WLAN_PHY_RXTA_CNTL_BIT_MASK QWLAN_PMU_CFG_AON_CNTL_MCU_SYSTEM_BOOT_COMPLETE_STATE_RESOURCE_REQ_PD_WLAN_PHY_RXA_CNTL_BIT_MASK
#endif
...
{
	uint32_t lut_map[11][2]={{100,100},{110,85},{120,80},{130,70},{140,60},{150,50},{160,40},{170,30},{180,20},{190,15},{200,10}};
	uint8_t i,j=0;
...
void nt_cc_battery_mgmt_init(void)
{
	...
	while(value);
		...
		*/
		...
		value |= ( 1 << QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_TIMEOUT_INT_EN_OFFSET) | ( 1 << QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_INIT_EN_OFFSET) |
...
float nt_pmu_vbatt_temp_get(type_t type)
{
	...
	if(type == TEMP_MEASURE)
	{
		...
		while(value);
		value = NT_REG_RD(QWLAN_PMU_TEMP_SNR_RD_DATA_REG);
		value = (uint32_t)(((value-LOWEST_TEMPERATURE_REG_READ)*TEMPERATURE_INCREMENT ) + LOWEST_TEMPERATURE_IN_CELSIUS);
	}
	else
	{
...
//-------------------------------------------------------------------------------------------------------------------
	...
	value = NT_REG_RD( QWLAN_TPE_INTERRUPT_ENABLE_REG );
	value |= (QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_TIMEOUT_INT_EN_MASK | QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_INT_EN_MASK) ;
	NT_REG_WR(QWLAN_TPE_INTERRUPT_ENABLE_REG,value );
 ...
 * <!-- nt_tpe_rint_get -->
 *
 * @brief            : for debugging purpose we will take parameters vbatt_avg, sample_index,sample_ptr,recovery_time,vbatt_endtime,vbatt_starttime.
 * @param statustype :
 * @return           : void
 */
uint32_t nt_tpe_rint_get(statustype_t statustype)
{
	...
	{
		value = NT_REG_RD(QWLAN_TPE_SW_VBATT_STATUS_REG & QWLAN_TPE_SW_VBATT_STATUS_VBATT_AVERAGE_MASK);//read the Current vbatt average.
	}

	else if(statustype == SAMPLE)
	{
        value |= ( 1 << QWLAN_TPE_SW_VBATT_STATUS_VBATT_SAMPLE_INDEX_OFFSET);//array of index use to display the samples.
		NT_REG_WR(QWLAN_TPE_SW_VBATT_STATUS_REG,value);
		value = NT_REG_RD(QWLAN_TPE_SW_VBATT_STATUS_REG & QWLAN_TPE_SW_VBATT_STATUS_VBATT_SAMPLE_MASK);//VBATT sample stored in array at index vbatt_sample_index.
	}
	else if(statustype == SAMPLE_PTR)
	{
		value = NT_REG_RD(QWLAN_TPE_SW_VBATT_STATUS_REG & QWLAN_TPE_SW_VBATT_STATUS_VBATT_HEAD_PTR_MASK);//Current head ptr for vbatt sample array.
	}
	else if(statustype == RECOVERY_TIME)
	{
		value = NT_REG_RD(QWLAN_TPE_SW_RINT_VBATT_STATUS_REG & QWLAN_TPE_SW_RINT_VBATT_STATUS_RINT_RECOVERY_TIME_MASK);//Read recovery time.
	}
	else if(statustype == VBATT_END_TIME)
	{
		value = NT_REG_RD(QWLAN_TPE_SW_RINT_VBATT_STATUS_REG & QWLAN_TPE_SW_RINT_VBATT_STATUS_VBATT_END_MASK);//read battery end voltage.
	}
	else if(statustype == VBATT_START_TIME)
	{
		value = NT_REG_RD(QWLAN_TPE_SW_RINT_VBATT_STATUS_REG & QWLAN_TPE_SW_RINT_VBATT_STATUS_VBATT_START_MASK);//read battery start voltage.
	}
	return value;
 ...
 * @param driver_fcn   : function pointer for the callback if the measured battery level is less than the required threshold
 ...
 *
 * @brief               : To calculate vbatt level and call the user defined function to adjust txop after interrupt has been triggered.
 ...
 * @param rint_user_config_t *user_config :  Rint measurement window in micro seconds
                                            ...
                                            uint32_t    rint_vbatt1_threshold;
                                             ...
                                             configuration RINT will only be measured if we have received more than this many samples (max 8)
                                            ...
                                            * 3 : Average taken over last 8 samples
                                            ...
                                            uint32_t    vbatt_timeout_threshold;
 * @return           : void
 */
void nt_issue_batt_level_mon_request(nt_cc_user_config_t *user_config,txop_driver_fcn driver_fcn)
{
	NT_REG_WR(QWLAN_TPE_SW_RINT_VBATT1_REG,user_config->rint_window<<QWLAN_TPE_SW_RINT_VBATT1_RINT_VBATT_WINDOW_OFFSET |
			user_config->rint_vbatt1_threshold);
	NT_REG_WR(QWLAN_TPE_SW_RINT_VBATT_REG,(QWLAN_TPE_SW_RINT_VBATT_RINT_VBATT_WINDOW_MON_EN_MASK |
	...
	nt_cc_add_client_callback(driver_fcn,user_config->vbatt_threshold ); // TODO :convert vbatt_threshold  to micro seconds before passing to the linked list
...
void nt_cc_battery_level(void)
{
	...
	rint_recovery_time = (rint_recovery_time & QWLAN_TPE_SW_RINT_VBATT_STATUS_RINT_RECOVERY_TIME_MASK)>>QWLAN_TPE_SW_RINT_VBATT_STATUS_RINT_RECOVERY_TIME_OFFSET;
	nt_cc_call_user_callback_from_isr( rint_recovery_time); // TODO convert it into micro seconds before passing it to the linked list
 ...
 void nt_update_battery_level(void)
 {
	...
	NT_REG_WR(QWLAN_TPE_INTERRUPT_ENABLE_REG,QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_TIMEOUT_INT_EN_MASK | QWLAN_TPE_INTERRUPT_ENABLE_RINT_VBATT_INT_EN_MASK);
  ...
  * <!-- nt_cc_txop_adjust -->
  *
  * @brief      : To abandon the txop if the battery level drops below a certain threshold. And, re configure the threshold value.
  * @return     : void
  */
