...
static uint16_t g_otp_version_word = 0;
...
static uint32_t g_wifi_ss_delay_2 = 120;
...
void hal_mac_sw_powerup()
{
	...
	for (uint32_t i = 0; i < g_wifi_ss_delay_2; i++)
    {
		mac_gdscr = HAL_REG_RD(QWLAN_PMU_WLAN_MAC_GDSCR_REG);
	...
	NT_LOG_HAL_ERR("mac_on_polling timeout", g_wifi_ss_delay_2,0,0);
}

void hal_mac_hw_ctrl()
{
	HAL_REG_WR(QWLAN_PMU_WLAN_MAC_GDSCR_REG, QWLAN_PMU_WLAN_MAC_GDSCR_DEFAULT);
}

uint16_t otp_version_otp_read()
{
	uint32_t OTP_TAG_HIGH = 0, OTP_TAG_LOW = 0;
    if (g_otp_version_word != 0) return g_otp_version_word;
#if(FERMION_CHIP_VERSION == 1)
...
#elif (FERMION_CHIP_VERSION == 2)
#if FERMION_CHIP_VERSION_MINOR == 0
	OTP_TAG_HIGH = HWIO_INXF(SEQ_WCSS_OTP_OFFSET, FERMION_V2_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_PTE_REGION_1_W3, TRIM_TAG_HIGH);
    OTP_TAG_LOW = HWIO_INXF(SEQ_WCSS_OTP_OFFSET, FERMION_V2_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_PTE_REGION_1_W3, TRIM_TAG_LOW);
#elif FERMION_CHIP_VERSION_MINOR == 1
	OTP_TAG_HIGH = HWIO_INXF(SEQ_WCSS_OTP_OFFSET, FERMION_V2_1_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_PTE_REGION_1_W3, TRIM_TAG_HIGH);
    OTP_TAG_LOW = HWIO_INXF(SEQ_WCSS_OTP_OFFSET, FERMION_V2_1_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_PTE_REGION_1_W3, TRIM_TAG_LOW);
#endif
#endif
    g_otp_version_word = (OTP_TAG_HIGH & 0xFF) << 8 | (OTP_TAG_LOW & 0xFF);
    return (uint16_t)g_otp_version_word;
}

#define R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0_TSENS_MON_VCM_TRIM_BMSK 0x3F000
#define R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0_TSENS_MON_VCM_TRIM_SHFT 12
#define R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3_ULP_BG_IPT_TRIM_BMSK 0xFF00
#define R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3_ULP_BG_IPT_TRIM_SHFT 8

void phyrf_temperature_data_otp_trim(void)
{
    uint32_t data;
#if(FERMION_CHIP_VERSION == 1)
    if (otp_version_otp_read() >= 0x0200)// for OTP >= v2
#elif (FERMION_CHIP_VERSION == 2)
    if (otp_version_otp_read() >= 0x0100)// for OTP >= v2
#endif
    {
        //#0x001A0140, 17:12 => 0x02043094, 11:6
#if(FERMION_CHIP_VERSION == 1)
      data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V1_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0);
#elif (FERMION_CHIP_VERSION == 2)
#if FERMION_CHIP_VERSION_MINOR == 0
	  data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V2_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0);
#elif FERMION_CHIP_VERSION_MINOR == 1
	  data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V2_1_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0);
#endif
#endif
        data = data & R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0_TSENS_MON_VCM_TRIM_BMSK;
        data = data >> R_QFPROM_RAW_RF_CALIBRATION_ROW8_W0_TSENS_MON_VCM_TRIM_SHFT;
        HWIO_OUTXF(SEQ_WCSS_RPMU_OFFSET, RPMU_RPMU_R_VCM_TRIM, TSENS_MON_VCM_TRIM, data);
        //#0x001A013C, 15:8 => 0x0204303C, 7:0
#if(FERMION_CHIP_VERSION == 1)
        data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V1_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3);
#elif (FERMION_CHIP_VERSION == 2)
#if FERMION_CHIP_VERSION_MINOR == 0
		data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V2_0_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3);
#elif FERMION_CHIP_VERSION_MINOR == 1
		data = HWIO_INX(SEQ_WCSS_OTP_OFFSET, FERMION_V2_1_QFPROM_RAW_FUSE_MAP_SECURITY_CONTROL_CORE_RAW_R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3);
#endif
#endif
        data = data & R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3_ULP_BG_IPT_TRIM_BMSK;
        data = data >> R_QFPROM_RAW_RF_CALIBRATION_ROW7_W3_ULP_BG_IPT_TRIM_SHFT;
        HWIO_OUTXF(SEQ_WCSS_RPMU_OFFSET, RPMU_RPMU_R_PMU_ULPBG_1, ULP_BG_IPT_TRIM, data);
    }
    else
    {
        HWIO_OUTXF(SEQ_WCSS_RPMU_OFFSET, RPMU_RPMU_R_VCM_TRIM, TSENS_MON_VCM_TRIM, 38);  //35~40
        HWIO_OUTXF(SEQ_WCSS_RPMU_OFFSET, RPMU_RPMU_R_PMU_ULPBG_1, ULP_BG_IPT_TRIM, 50);  //50
    }
}
...
void wifi_fw_pmic_init(cpr_mode_e cpr_mode)
{
    ...
    if (g_ferm_neutrino_mode == 1) {
        ...
        HWIO_OUTXF(SEQ_WCSS_RPMU_OFFSET, RPMU_RPMU_R_PMU_SMPS1_5, SMPS1_LPM_OVR, 3); // follows TX enable signal from
                                                                                     // PHY
