...
uint32_t nt_sysmon_batt_voltage_get(void){
	...
	//snprintf((char *)str,sizeof(str),"%s" "%u" "%s" "%u" "%s","Neutrino Voltage",(int16_t)itemp_volt,".",(int16_t)itemp_volt0,"\r\n");
	//nt_dbg_print(str);
#endif
	return Voltage*100 ;
}
...
void pmu_ccpu_vbat_low_hit_int_p (void)
{
	...
	NVIC_ClearPendingIRQ(PMU_ccpu_vbat_low_hit_int_p);
#ifdef NT_DEBUG
	//dbg_port
...
void nt_sysmon_threshold_init(void){
	...
	qurt_isr_register_3(PMU_ccpu_vbat_low_hit_int_p, pmu_ccpu_vbat_low_hit_int_p);
    NVIC_EnableIRQ(PMU_ccpu_vbat_low_hit_int_p);
	...
	snprintf((char *)str,sizeof(str),"panic high temperature limit %d \r\n",(uint32_t)threshold_int_status);
	//nt_dbg_print(str);
#endif
	...
	if (ret_val != NT_OK)
	{
		uart_hal_poll_out_str_ext("system task init failed");
	}
#endif
}
...
static void nt_sysmon_main_task(){
	...
	for(;;){
		...
		qurt_thread_sleep_ms(*((uint32_t*)(nt_devcfg_get_config(NT_DEVCFG_SYSMON_TASK_PERIODICITY_CONFIG))));
	}
}
...
int32_t nt_sysmon_task_create(void)
{
	...
	ret_val = nt_qurt_thread_create(nt_sysmon_main_task, "sysmon_task", NT_SYSMON_STACK_SIZE, NULL, NT_SYSMON_TASK_PRIO, &nt_sysmon_hnd);
	return (ret_val);
}
...
void nt_system_sw_reset( void )
{
	...
	uart_hal_poll_out_str_ext("Software Reset\r\n");
    ...
    if (reset_dbg) {
        ...
        uint32_t value = 0;
        value |= (reset_dbg&0x1)?(3 << QWLAN_PMU_CFG_IO_BASED_STRAP_OVERRIDE_VALUE_CFG_AON_TESTBUS_ENABLE_OFFSET):0;
        value |= (reset_dbg&0x2)?(1 << QWLAN_PMU_CFG_IO_BASED_STRAP_OVERRIDE_VALUE_CFG_TEST_STRAP_OFFSET):0;
        value |= (reset_dbg&0x4)?(1 << QWLAN_PMU_CFG_IO_BASED_STRAP_OVERRIDE_VALUE_CFG_RRAM_BYPASS_ENABLE_OFFSET):0;
