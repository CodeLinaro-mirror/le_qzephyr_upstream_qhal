...
#define _MEM_RRAM_MAIN_ADDR_CHK(addr) ((addr >= g__rram_region_start_addr) && (addr < g__rram_region_end_address))
#define _MEM_RRAM_OTP_ADDR_CHK(addr)  ((addr >=g__OTP_region_st_addr) && (addr < g__OTP_region_end_addr))

#define _MEM_TX_DUR_WR ( (NT_REG_RD(QWLAN_RRAM_CTRL_RRAM_TX_DUR_WR_REG) & QWLAN_RRAM_CTRL_RRAM_TX_DUR_WR_STATUS_MASK) ? true : false )
#define _MEM_WR_DUR_TX ( (NT_REG_RD(QWLAN_RRAM_CTRL_RRAM_WR_DUR_TX_REG) & QWLAN_RRAM_CTRL_RRAM_WR_DUR_TX_STATUS_MASK) ? true : false )
...
#define RRAM_R_W_BUFF_SIZE 256 /* buffer used for padding / partial write case. Must be multiple of 16 byte. This can be adjusted for PBL/SBL/APP */
#define RRAM_WRITE_BLOCK_MAX_SIZE 16368 /* this is maximum number of bytes DXE can write using a discriptor. it is 0x3FF0 */
...
static uint8_t address_to_region_num_map(uint32_t address)
{
	...
	region_end_addr = g__OTP_region_st_addr;
...
#ifdef FERMION_OTP_SUPPORT
static inline int8_t rram_address_range_check( uintptr_t rram_add, uint32_t data_len, nt_otp_region_per_status rd_wr_lock )
#else
...
#endif
{
    ...
    if(_MEM_RRAM_OTP_ADDR_CHK(rram_add) && _MEM_RRAM_OTP_ADDR_CHK(rram_add + data_len))
    {
        ...
        {
            err_printf("region check failed\r\n");
            return(-EINVAL);
        }
    }
    else
	...
	uint32_t length)
{
    ...
    {
        NT_LOG_PRINT(COMMON,ERR,"RRAM dst address or source is not algined dst 0x%X src 0x%X",dst,(uint32_t)src_adr);

        /* go back in RRAM address so that it is algined to RRAM_WRITE_ADR_BYTE_ALIGN */
        des_adr = des_adr - (dst % RRAM_WRITE_ADR_BYTE_ALIGN); /* destimation adress is RRAM_WRITE_ADR_BYTE_ALIGN algined */
        ...
        for(count = (dst % RRAM_WRITE_ADR_BYTE_ALIGN); count < (dst % RRAM_WRITE_ADR_BYTE_ALIGN) + temp_num_byte; count++)
        {
        ...
        {
            /* RRAM write failed */
            NT_LOG_PRINT(COMMON,ERR,"RRAM write failed for non aligned destination address %d src 0x%X dst 0x%X",RRAM_WRITE_ADR_BYTE_ALIGN, (uint32_t)temp_r_w_buff, des_adr);
            return(status);
        }
        ...
        if(remaining_byte)
        {
            ...
            src_adr = src_adr + temp_num_byte; /* source address may or may not be aligned and based on that write path will change */
            ...
            if(((uint32_t)src_adr%4) != 0)
            {
                ...
                for(count = 0; count < full_block_num; count++)
                {
                    ...
                    {
                        /* RRAM write failed */
                        NT_LOG_PRINT(COMMON,ERR,"RRAM special case write failed for block %d src 0x%X dst 0x%X",count, (uint32_t)temp_r_w_buff, des_adr);
                        return(status);
                    }
                    /* update the address */
                ...
                if(partial_block_size != 0)
                {
                    ...
                    {
                        /* RRAM write failed */
                        NT_LOG_PRINT(COMMON,ERR,"RRAM write of spacial case failed for padded partial block %d src 0x%X dst 0x%X",(partial_block_size), (uint32_t)temp_r_w_buff, des_adr);
                        return(status);
                    }
                }
    ...
    for(count = 0; count < full_block_num; count++)
    {
        ...
        {
            /* RRAM write failed */
            NT_LOG_PRINT(COMMON,ERR,"RRAM write failed for block %d src 0x%X dst 0x%X",count, (uint32_t)src_adr, des_adr);
            return(status);
        }
        /* update the address */
    ...
    if(partial_block_size != 0)
    {
        ...
        if(num_pad_bytes == 0)
        {
            ...
            {
                /* RRAM write failed */
                NT_LOG_PRINT(COMMON,ERR,"RRAM write failed for partial block %d src 0x%X dst 0x%X",partial_block_size, (uint32_t)src_adr, des_adr);
                return(status);
            }
        ...
        else
        {
            ...
            /* if partial block size is 200 bytes then 12*16 = 192 bytes will be written first and rest 8 bytes would be written with padding */
            ...
            temp_num_byte = temp_num_byte * RRAM_WRITE_ADR_BYTE_ALIGN; /* NUM bytes which are multiple of RRAM_WRITE_ADR_BYTE_ALIGN */
            ...
            if(temp_num_byte != 0)
            {
                ...
                {
                    /* RRAM write failed */
                    NT_LOG_PRINT(COMMON,ERR,"RRAM write failed for padded partial block %d src 0x%X dst 0x%X",(partial_block_size), (uint32_t)src_adr, des_adr);
                    return(status);
                }
            }
...
//    uint32_t temp_regVal[6];
    ...
    if(verify_write_through_read)
    {
        ...
        if(error_count)
        {
            NT_LOG_PRINT(COMMON,ERR,"RRAM write failed length %d src 0x%X dst 0x%X error count %d",length, (uint32_t)src_adr, dst, error_count);
            status = -EBADE;
        }
        else
        {
            NT_LOG_PRINT(COMMON,INFO,"RRAM write passed length %d src 0x%X dst 0x%X error count %d",length, (uint32_t)src_adr, dst, error_count);
        }
...
#define RRAM_BLOCK_LEN (128/8) // RRAM blocks are 128 bits
  ...
  else
  {
	  ...
	  for (block_addr = first_block_addr; block_addr <= last_block_addr; block_addr += RRAM_BLOCK_LEN)
	  {
			...
			{
				//if(_rram_write_status((uintptr_t)(block_addr + (word_idx * sizeof(word_idx))), dst_blk.word[word_idx]) != NT_MEM_SUCCESS)
...
#endif //NT_FN_QSPI_FLASH
			#ifdef NT_DEBUG
			err_printf("RRAM Invalid address\r\n");
			#endif
			return (-EINVAL);
		}
#if (NT_CHIP_VERSION==2) || defined(PLATFORM_FERMION)
...
nt_macid_write(
	uint8_t *macbuf )
{
        NT_LOG_PRINT(COMMON,INFO,"requested mac adr is  0x%X 0x%X 0x%X 0x%X 0x%X 0x%X",macbuf[0], macbuf[1], macbuf[2], macbuf[3], macbuf[4], macbuf[5]);
        ...
        if((macbuf[0] & 0x3) != 0)
        {
            ...
            NT_LOG_PRINT(COMMON,ERR,"setting global / multicast bit in mac address is not allowed clearing the same 0x%X",macbuf[0]);
        }
...
nt_reset_caldata(
	void )
{
	uint32_t *address = (uint32_t *)g_ln_CAL_Start_Addr;
	uint32_t cal_length = g_ln_CAL_Data_length;
	...
	for ( uint16_t rst_count = 0; rst_count < (cal_length >> 4); rst_count++ )
	{
		...
		{
			#ifdef NT_DEBUG
				err_printf("CAL data reset failed\r\n");
			#endif
		}
...
{
	int8_t wcal_err = -1;
	uint32_t *cal_address = (uint32_t*)g_ln_CAL_Start_Addr;
	uint32_t cal_length = g_ln_CAL_Data_length;
...
static uint8_t
_mem_cmem_validate(
	uint32_t *cMEM_st_addr,
	uint32_t *cMEM_mid_addr)
{
...
static void
_mem_cmem_on_test(
	uint32_t bt_cmpl_bit_mask, // bit mask for the specified bank
	...
	uint8_t  tst_flag)         // bank init and test required = 1, no test = 0
{
	...
	do
	{
		...
		uint32_t st_value  = (NT_REG_RD(_MEM_SYSTICK_CTRL_ADDR) & (1 << 16)); // Reading the COUNTFLAG bit of systick control register.
		if (st_value)
		{
	...
	uint8_t test_flag)
{
	_mem_cmem_on_test(QWLAN_PMU_CFG_AON_CNTL_MCU_SYSTEM_BOOT_COMPLETE_STATE_RESOURCE_REQ_PD_CMEM_BANK_B_CNTL_BIT_MASK,
	                  QWLAN_PMU_CMEM_BANK_B_GDSCR_REG,
	                  (uint32_t *) &_ln_cMEM_Bank_B_start_addr__,
	...
	_mem_cmem_on_test(QWLAN_PMU_CFG_AON_CNTL_MCU_SYSTEM_BOOT_COMPLETE_STATE_RESOURCE_REQ_PD_CMEM_BANK_C_CNTL_BIT_MASK,
	                  QWLAN_PMU_CMEM_BANK_C_GDSCR_REG,
	                  (uint32_t *) &_ln_cMEM_Bank_C_start_addr__,
					  test_flag); // bank test required
	_mem_cmem_on_test(QWLAN_PMU_CFG_AON_CNTL_MCU_SYSTEM_BOOT_COMPLETE_STATE_RESOURCE_REQ_PD_CMEM_BANK_D_CNTL_BIT_MASK,
	                  QWLAN_PMU_CMEM_BANK_D_GDSCR_REG,
	                  (uint32_t *) &_ln_cMEM_Bank_D_start_addr__,
					  test_flag); // bank test required
}

static uint8_t
_mem_cmem_pwr_mode(
	Mem_Control pwr_mode,
	uint32_t dig_top_mask,
	uint32_t gdscr,
	uint32_t boot_cmpl_mask)
{
	uint32_t reg_value;
	uint8_t  ret = NT_OK;
	...
	if (pwr_mode == Retention)
	{
        ...
        */
        if(dig_top_mask == 1)
        {
            NT_REG_WR(QWLAN_PMU_CFG_CMEM_BANK_A_RET_EN_REG,
            QWLAN_PMU_CFG_CMEM_BANK_A_RET_EN_DEFAULT);
        }
#else
	...
	Mem_Control pwr_mode)
{
	...
	{
		case 1 :
			ret = _mem_cmem_pwr_mode(pwr_mode,
                    #ifdef FR_HWIO_WAR
					1,
                    #else
...
{
    otp_nt_region * const nt_otp_reg = (otp_nt_region *)g__OTP_region_st_addr;
    uint32_t lock_idx;
    uint8_t lock_byte;
    uint8_t *lock_addr;
