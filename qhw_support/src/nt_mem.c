...
static void _mem_systick_timer_stop(void) { NT_REG_WR(_MEM_SYSTICK_CTRL_ADDR, _MEM_SYSTICK_STOP_VAL); }
...
void nt_enable_disable_rram_dxe(uint8_t rram_dxe_flg) { rram_dxe_status = rram_dxe_flg; }
uint8_t nt_status_enable_disable_rram_dxe(void) { return rram_dxe_status; }
#endif //(NT_CHIP_VERSION==2)|| defined(PLATFORM_FERMION)
