...
nt_osal_semaphore_handle_t ntSleepSemaphoreHandle;
 ...
 */
const ip_addr_t *
nt_dpm_get_ip(struct netif *netif, u8_t type, s8_t idx)
{
 ...
 */
void
nt_dpm_set_ip(struct netif *netif, ip_addr_t *ip, s8_t idx)
{
...
void
nt_dpm_set_ip4(struct netif* netif, ip_addr_t *ip_addr, ip_addr_t *netmask, ip_addr_t *gw)
{
...
void
nt_dpm_free_network_buffer(void *buf)
{
    struct pbuf *p;
    uint32_t ret;
...
void
nt_dpm_realloc_network_buffer(void *buf, uint32_t length)
{
    struct pbuf *p;
...
nt_dpm_forward_eth_packet_to_stack(p_ndpA ad, void *rx_frame, void *eth_frame, uint32_t length, device_t *dev)
{
    ...
    if (p != NULL) {
        ...
        if ((nt_dpm_tm.rx_stat[NETIF_INPUT].valid == 0) && (*(uint32_t *)((uint8_t *)eth_frame + nt_dpm_tm.rx_stat[RX_INTERRUPT].offset) == nt_dpm_tm.rx_marker)) {
...
#else
     if ((pnDpA->wmm_enable == NT_DPM_WMM_ENABLE) && ((eth_hdr->usFrameType == dp_htons(NT_IP_FRAME_TYPE)) || (eth_hdr->usFrameType == dp_htons(NT_IPV6_FRAME_TYPE))))
...
static NT_BOOL nt_dpm_copy_ip_params(device_t *dev, void* buffer)
{
    ...
    if(IPADDR_TYPE_V4 == ringif_ip_config->ip_ver)
    {
        ...
        IP6_ADDR(p_ip6, ringif_ip_config->ipv6_addr[0],ringif_ip_config->ipv6_addr[1],
                                   ringif_ip_config->ipv6_addr[2],ringif_ip_config->ipv6_addr[3]);
#else /* LWIP_IPV6 */
...
static void nt_dpm_send_if_add_event(device_t *dev, void* buffer, struct netif *netif)
{
    ...
    if(g_Cmd_Translation_wifi_hndl.msg_struct.event_notify)
    {
        ...
        g_Cmd_Translation_wifi_hndl.msg_struct.event_notify(eWiFiSuccess,
            wifi_interface_add_event_id, &if_add_evt);
    }
    else
    {
...
static bool nt_dpm_ip_addr_ready_ind(uint8_t netif_id, uint8_t dhcp_type, uint8_t ip_ver, uint32_t ipv4_addr, const uint32_t *ipv6_addr)
{
...
static void nt_dpm_netif_dhcp_hdlr(struct netif* p_netif)
{
    if (p_netif != NULL)
    {
        ...
        else if ((netif_dhcp6_data(p_netif) != NULL))
        {
            ...
            if ((dhcp_struct->state == DHCP_STATE_BOUND) ||
            (dhcp_struct->state == DHCP_STATE_RENEWING) ||
 ...
 */
void
nt_dpm_remove_dev_from_stack (device_t *dev)
{
#ifdef LWIP
    struct netif *netif;
#if NT_FN_DHCPS_V4 && LWIP_DHCP
 ...
 */
uint8_t
nt_dpm_enable_disable_dhcp(struct netif *netif, NT_BOOL is_server, uint8_t state, __unused uint8_t cmd_cli)
{
    ...
    Default:
    netif_set_addr(netif, (const ip4_addr_t*)ip_2_ip4(&_ip_address[role]), (const ip4_addr_t*)ip_2_ip4(&netmask[role]),
...
void
nt_dpm_netif_set_link_up(struct netif * netif)
{
...
#else
void
nt_dpm_netif_set_link_down(struct netif * netif)
{
