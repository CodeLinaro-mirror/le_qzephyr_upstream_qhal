...
__attribute__ ((section(".perf_rx_txt"))) uint8_t nt_dpm_forward_eth_packet_to_stack(p_ndpA ad, void *rx_frame, void *eth_frame, uint32_t length, device_t *dev);

#ifdef SUPPORT_RING_IF
__attribute__ ((section(".perf_nc_txt"))) void nt_dpm_add_dev_to_stack(device_t *dev, void *buffer);
else
__attribute__ ((section(".perf_nc_txt"))) void nt_dpm_add_dev_to_stack(device_t *dev);
...
#ifdef SUPPORT_RING_IF
__attribute__ ((section(".perf_nc_txt"))) nt_status_t nt_dpm_process_eth_packet_from_stack(void *frame, uint32_t length, uint8_t qos_for_non_ip);
#else
...
__attribute__ ((section(".perf_nc_txt"))) void nt_dpm_set_ip4(struct netif* netif, ip_addr_t *ip_addr, ip_addr_t *netmask, ip_addr_t *gw);
__attribute__ ((section(".perf_nc_txt"))) uint8_t nt_dpm_enable_disable_dhcp(struct netif *netif, NT_BOOL is_server, uint8_t state, uint8_t cmd_cli);
