...
{
	hal->UART_UART_RBR.reg =
		((dll << UART_RBR_THR_DLL_REG_RBR_POS) & UART_RBR_THR_DLL_REG_DLL_MSK);
	return;
}
...
{
	hal->UART_UART_IIR.reg |=
		((enable << UART_IIR_FCR_REG_FIFOE_POS) & UART_IIR_FCR_REG_FIFOE_MSK);
	return;
}
