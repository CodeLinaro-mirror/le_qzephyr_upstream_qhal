...
typedef enum {
	...
	// Above status are keep align with the previous project, some may not be used in the
	// Fermion
...
#define HCNT_CAL(period, freq, spklen)                                                             \
	(((period - SCL_FALL_TIME) * freq + 500000) / 1000000 - spklen - 7)

#define LCNT_CAL(period, freq)                                                                     \
	(((period + SCL_FALL_TIME - SCL_RISE_TIME) * freq + 500000) / 1000000 - 1)
...
#endif // CORE_SYSTEM_INC_FERM_I2C_H_
