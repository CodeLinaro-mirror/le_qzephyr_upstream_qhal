...
typedef struct flash_config_data_s {
	...
	uint8_t read_cmd_mode;  /**< Read operation command mode, QAPI_FLASH_RW_MODE_XXX. QUP access
				   only supports single and quad mode. */
	uint8_t read_addr_mode; /**< Read operation address mode, QAPI_FLASH_RW_MODE_XXX. QUP access
				   only supports single and quad mode. */
	uint8_t read_data_mode; /**< Read operation data mode, QAPI_FLASH_RW_MODE_XXX. QUP access
				   only supports single and quad mode. */
	...
	uint8_t bulk_erase_opcode; /**< The opcode for the supported bulk erase size (larger than a
				      4KB block erase). If bulk erase is not supported, set to 0. */
	uint8_t chip_erase_opcode; /**< Opcode used for chip erase. */
	uint8_t quad_enable_mode;  /**< Quad Enable Requirements (QER) as defined in the JEDEC
				      Standard No. 216A Document. */
	...
	uint8_t erase_err_bmsk; /**< Status BIT(s) in the EraseErrStatusReg register; indicating if
				   there is an erase error condition. */
	uint8_t erase_err_status_reg; /**< Register address used for polling the erase status. */
	uint8_t write_err_bmsk; /**< Status BIT(s) in the WriteErrStatusReg register; indicating if
				   there is a write error condition. */
	uint8_t write_err_status_reg; /**< Register address used for polling the write status. */
	uint8_t high_performance_mode_bmask; /**< High Performance Bit(s). Non-Macronix parts set to
					       0. This field is used to enable High Performance mode
					       supported on some Macronix parts. */
	uint8_t power_on_delay_in_us;        /**< Power On Reset delay in 100us units. */
	uint16_t suspend_erase_delay_in_us; /**< Delay needed after suspending an in-progress erase;
					       see JEDEC Standard No. 216A for a definition. */
	uint16_t suspend_program_delay_in_us; /**< Delay needed after suspending an in-progress
						 program; see JEDEC Standard No. 216A for a
						 definition. */
	uint16_t resume_erase_delay_in_us; /**< Delay needed after sending a resume erase; see JEDEC
					      Standard No. 216A for a definition. */
	uint16_t resume_program_delay_in_us; /**< Delay needed after sending a resume program; see
						JEDEC Standard No. 216A for a definition. */
