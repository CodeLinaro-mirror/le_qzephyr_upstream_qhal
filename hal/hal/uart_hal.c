...
{
	uart_ll_parity_set(hal, parity);
}

int uart_hal_baudrate_set (uart_hal *hal, uint32_t baudrate)
{
	return uart_ll_baudrate_set(hal, baudrate);
}
...
{
    uart_hal_poll_out(g_uart_hal, ch);
}
...
{
   uart_hal_poll_outs(hal, str, strlen(str));
}

void uart_hal_poll_out_str_ext(const char *str)
{
    uart_hal_poll_out_str(g_uart_hal, str);
}

void uart_hal_enable_intr_rx_ext (void)
{
    uart_hal_enable_intr_rx(g_uart_hal);
}
