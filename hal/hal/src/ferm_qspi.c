...
*/
static void drv_qspi_pio_read(uint8_t *buffer, uint32_t num_bytes, uint8_t write, uint8_t io_mode,
			      uint8_t fragment)
{
	uint8_t *ptr = buffer;
...
/**
   ...
   @param[in] pio_transfer_config  PIO transfer configurations.
*/
static void drv_qspi_pio_write(uint8_t *buffer, uint32_t num_bytes, uint8_t write, uint8_t io_mode,
			       uint8_t fragment)
{
	uint8_t *ptr = buffer;
	uint32_t bytes_left = num_bytes;
...
static void drv_qspi_pio_transfer_chain(qspi_descriptor_t *pchain)
{
	...
	while (pdesc) {
		...
		if (write) {
			drv_qspi_pio_write(buffer, num_bytes, write, pdesc->multi_io_mode,
					   pdesc->fragment);
		} else {
			drv_qspi_pio_read(buffer, num_bytes, write, pdesc->multi_io_mode,
					  pdesc->fragment);
		}
...
*/
static qspi_descriptor_t *drv_qspi_alloc_descriptor(uint32_t bytes_needed,
						    qspi_transfer_mode_t transfer_mode)
{
	...
	if (room < bytes_needed || next == NULL) {
		...
		drv_qspi_flush_chain((transfer_mode != QSPI_DMA_INT_MODE_E) ? transfer_mode
									    : QSPI_DMA_POLL_MODE_E);
...
/**
   ...
   @param[in] transfer_mode  QSPI transfer mode.
*/
static void drv_qspi_queue_opcode(uint8_t opcode, qspi_mode_t cmd_mode,
				  qspi_transfer_mode_t transfer_mode)
{
				...
				qspi_transfer_mode_t transfer_mode)
{
	...
	drv_qspi_set_descriptor(write_addr, (uint32_t)addr_bytes, addr_mode, QSPI_MASTER_WRITE_E,
				transfer_mode);
}
				...
				bool write, qspi_transfer_mode_t transfer_mode)
{
	...
	prolog_bytes = ((uint32_t)(aligned_ptr - data) > data_bytes)
			       ? (data_bytes)
			       : (uint32_t)(aligned_ptr - data);
	epilog_bytes = ((transfer_mode != QSPI_PIO_MODE_E) && !write)
			       ? ((data_bytes - prolog_bytes) & 0x3)
			       : 0;
	...
	if (prolog_bytes) {
		drv_qspi_set_descriptor(data, prolog_bytes, data_mode, (qspi_transfer_dir_t)write,
					transfer_mode);
	}
	if (aligned_bytes) {
		drv_qspi_queue_data_direct(aligned_ptr, aligned_bytes, data_mode, write,
					   transfer_mode);
	}
	if (epilog_bytes) {
		drv_qspi_set_descriptor(epilog_ptr, epilog_bytes, data_mode,
					(qspi_transfer_dir_t)write, transfer_mode);
	}
}
...
bool drv_qspi_init(qspi_master_config_t *config)
{
	...
	pads_option = 1; /// CONFIG_BOARD_QCC730_QSPI_V2_QUAD_MODE;
	...
	if (!hal_qspi_is_qspi_active()) {
		...
		if (!hal_qspi_xip_is_enabled(qspi_context.hal)) {
			/* reset registers */
			/* TODO: if XIP is enabled in bootloader, we need a careful reset, now use
			 * default value.*/
...
bool drv_qspi_prepare_cmd(qspi_cmd_t *cmd, uint8_t opcode, uint8_t addr_bytes, uint8_t dummy_clocks,
			  qspi_mode_t cmd_mode, qspi_mode_t addr_mode, qspi_mode_t data_mode,
			  bool write)
{
	if (cmd == NULL) {
		return false;
...
int32_t drv_qspi_xip_config(qspi_xip_flash_region region_id, uint32_t region_size,
			    uint32_t regigon_addr)
{
	 ...
	 * 4K bytes/block
	 */
	hal_qspi_xip_region_config(qspi_context.hal, region_id, region_size,
				   (regigon_addr / BLOCK_SIZE_IN_BYTES));
