...
static inline uint32_t uart_hal_tx_empty(uart_hal *hal)
{
	return (hal->UART_UART_LSR.bit.TEMPT);
}
...
{
	hal->UART_UART_RBR.reg = ((dll << UART_RBR_THR_DLL_REG_RBR_POS)
									& UART_RBR_THR_DLL_REG_DLL_MSK);
	return;
}

static inline void uart_hal_divisor_high_cfg(uart_hal *hal, uint32_t dlh)
{
	hal->UART_UART_DLH.reg = ((dlh << UART_IER_DLH_REG_DLH_POS)
									& UART_IER_DLH_REG_DLH_MSK);
	return;
}
...
{
	hal->UART_UART_IIR.reg |= ((enable << UART_IIR_FCR_REG_FIFOE_POS)
									& UART_IIR_FCR_REG_FIFOE_MSK);
	return;
}

static inline void uart_hal_fifo_tet_cfg(uart_hal *hal, uint32_t tet)
{
	hal->UART_UART_IIR.reg |= ((tet << UART_IIR_FCR_REG_TET_POS)
								& UART_IIR_FCR_REG_TET_MSK);
	return;
}

static inline void uart_hal_fifo_rt_cfg(uart_hal *hal, uint32_t rt)
{
	hal->UART_UART_IIR.reg |= ((rt << UART_IIR_FCR_REG_RT_POS)
								& UART_IIR_FCR_REG_RT_MSK);
	return;
}
