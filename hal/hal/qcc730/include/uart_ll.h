...
static inline void uart_hal_enable_intr_rx(uart_hal *hal) { hal->UART_UART_DLH.reg |= UART_IER_DLH_REG_ERBFI_MSK; }

static inline void uart_hal_disable_intr_rx(uart_hal *hal) { hal->UART_UART_DLH.reg &= ~UART_IER_DLH_REG_ERBFI_MSK; }

static inline uint32_t uart_hal_intr_tx_busy(uart_hal *hal) { return (hal->UART_UART_USR.reg & UART_USR_REG_BUSY_MSK); }
...
static inline uint32_t uart_hal_tx_empty(uart_hal *hal) { return (hal->UART_UART_LSR.bit.TEMPT); }

static inline uint32_t uart_hal_intr_tx_done(uart_hal *hal) { return (hal->UART_UART_USR.reg & UART_USR_REG_TFE_MSK); }

static inline uint32_t uart_hal_intr_rx_full(uart_hal *hal) { return (hal->UART_UART_USR.reg & UART_USR_REG_RFF_MSK); }

static inline uint32_t uart_hal_intr_get(uart_hal *hal) { return (hal->UART_UART_IIR.reg); }

static inline uint32_t uart_hal_line_status_get(uart_hal *hal) { return hal->UART_UART_LSR.reg; }
...
static inline uint8_t uart_hal_rx_read(uart_hal *hal) { return hal->UART_UART_RBR.reg; }
